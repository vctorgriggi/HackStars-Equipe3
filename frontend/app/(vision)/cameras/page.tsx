    ...
    <div className="overflow-hidden rounded-lg border border-line bg-surface-1 shadow-1">
      ...
      >
        ...
        >
          <span
            aria-hidden
            className={`h-1.5 w-1.5 rounded-full ${camera.ativa ? "tv-pulse bg-reading-mismatch" : "bg-surface-3"}`}
          />
          {camera.nome}
        </span>
        <span
      ...
      <div className="flex items-center gap-2 px-3 py-2">
        ...
        <span className="text-xs text-text-2">
          {camera.ativa ? "Feed simulado nesta rodada" : "Câmera inativa"}
        </span>
      </div>
    </div>
  );
...
function FormNovaCamera({
  ...
  const podeEnviar = nome.trim().length > 0 && !criando;
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <input
          ...
          placeholder="Endpoint RTSP (opcional)"
          aria-label="Endpoint do stream (opcional)"
          className="t-mono h-9 min-w-0 flex-1 basis-44 rounded-md border border-line bg-surface-2 px-3 text-xs text-text-1 outline-none placeholder:text-text-3 focus-visible:[box-shadow:var(--ring-focus)]"
        />
        <button
          type="submit"
          disabled={!podeEnviar}
              ...
              {cameras.map((camera) => (
                ...
                >
                  ...
                  <Select
                    ...
                    onValueChange={(v) =>
                      atualizar.mutate({
                        ...
                        payload: {
                          fonteFisica:
                            v as (typeof FONTES_FISICAS_API)[number],
                        },
                      })
                    }
                    options={OPCOES_FONTE}
                  ...
                  <Select
                    ...
                    className="h-8 text-xs"
                  />
                  <span className="t-mono min-w-0 flex-[1_1_140px] truncate text-2xs text-text-3">
                    {camera.endpoint ?? "sem endpoint"}
                  </span>
                  ...
                  <button
                    type="button"
                    onClick={() => excluir.mutate(camera.id)}
                    disabled={excluir.isPending}
                    aria-label={`Excluir câmera ${camera.nome}`}
                    className="flex h-8 w-8 flex-none items-center justify-center rounded-md border border-line text-text-3 hover:bg-surface-3 hover:text-text-1 disabled:cursor-not-allowed disabled:opacity-50 focus-visible:outline-none focus-visible:[box-shadow:var(--ring-focus)]"
                  >
