...
@Injectable()
export class CamposConferidosService {
  constructor(
  ...
  async create(createCampoConferidoDto: CreateCampoConferidoDto) {
    ...
    if (createCampoConferidoDto.fotoEvidencia) {
      const fotoEvidenciaObject = await this.fotosEvidenciaService.findById(
  ...
  async criarComVeredito(dados: {
    ...
    if (dados.fotoEvidenciaId) {
      fotoEvidencia = await this.fotosEvidenciaService.findById(
